...
{
    [Info("Backpack", "DarkPlugins.ru", "1.0.4")]
    public class Backpack : RustPlugin
    {
            ...
            switch (arg.GetString(0))
            {
                ...
                case "craft":
                    ...
                    if (success)
                    {
                        ...
                        Item craft = ItemManager.CreateByName("hoodie", 1, _config.skinIdBackpack);
        ...
        private void chatCmdBackpack(BasePlayer player, string command, string[] args)
        {
            ...
            container.Add(new CuiElement()
            {
                ...
                Components =
                {
                    ...
                    {
                        Png = (string) ImageLibrary.Call("GetImage", "hoodie", _config.skinIdBackpack)
                    },
        ...
        private bool? CanWearItem(PlayerInventory inventory, Item item)
        {
            ...
            if (item.skin == _config.skinIdBackpack)
            {
                if (player.inventory.containerWear.itemList.Find(x => x.skin == _config.skinIdBackpack) != null)
                    return false;
                if (player.inventory.containerWear.itemList.Find(x => x.info.shortname == "hoodie") != null)
                    return true;
            }
            return null;
        }
        ...
        public class BackpackStorage : MonoBehaviour
        {
            ...
            {
                backpack.LockUnlock(state, player);

                foreach (var item in player.inventory.AllItems())
                    if (item.skin == ins._config.skinIdBackpack)
                        item.LockUnlock(state, player);
            }
        }
